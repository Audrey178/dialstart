...
| 2b | Run every `prompt` in `jobs.jsonl` through an LLM, append `{"job_id", "output", ...}` lines to `outputs.jsonl` (resumable) | `run_jobs.py` |
...
# pilot first (--limit 20), then the rest; either
python scripts/synth/run_jobs.py --jobs $V/jobs.jsonl --out $V/outputs.jsonl --backend anthropic --batch
# or a model served on the H200 with vLLM
python scripts/synth/run_jobs.py --jobs $V/jobs.jsonl --out $V/outputs.jsonl --backend openai \
